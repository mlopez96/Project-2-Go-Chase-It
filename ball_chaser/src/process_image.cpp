...
void process_image_callback(const sensor_msgs::Image img)
{
    ...
    if (!ball_frame)  // ball not in frame
    	drive_robot(0.0, 0.0);
    else
    {
      ...
      {
        drive_robot(0.0, 0.05);   // turn left bc white_pixel in left region
      ...
      {
      	drive_robot(0.125, 0.0);   // go forward
      ...
      {
        drive_robot(0.0, -0.05);   // turn right bc white_pixel in right region
